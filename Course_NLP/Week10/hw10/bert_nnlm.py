...
class LanguageModel(nn.Module):
    ...
    def forward(self, x, y=None):
        if y is not None:
            ...
            mask = torch.tril(torch.ones((x.shape[0], x.shape[1], x.shape[1]))) # torch.tril返回下三角矩阵, 1表示下三角，0表示上三角
            # x.shape[0], x.shape[1], x.shape[1] 是batch_size, seq_len, seq_len
...
if __name__ == "__main__":
    ...
    train(r"C:\Users\Gurkha\Desktop\BaDou\Code\Course_NLP\Week10\demo\lstm语言模型生成文本\corpus.txt", False)
