...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.loss = nn.functional.cross_entropy  # loss函数采用交叉熵损失，内部包含softmax
...
# 随机生成一个5维向量，标签为最大值的索引
# 在使用 PyTorch 的交叉熵损失函数 nn.functional.cross_entropy 时，
# 目标标签 y 应该是类别的索引，而不是 one-hot 编码的向量。
def build_sample():
...
def evaluate(model):
    ...
    print(f"本次预测集中有 {len(x)} 个样本数量，{len(y)} 个标签数量")
    with torch.no_grad():
        y_pred = model(x)  # 模型预测，输出 logits
        y_pred_label = torch.argmax(y_pred, dim=1)  # 获取预测的类别索引
        correct = (y_pred_label == y).sum().item()  # 计算预测正确的数量
