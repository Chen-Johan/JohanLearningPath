...
from model import TripletLossNetwork, choose_optimizer
...
class Predictor:
    ...
    def predict(self, sentence):
        input_id = self.encode_sentence(sentence)   #将输入句子转化成id
        input_id = torch.LongTensor([input_id])     #转化成tensor
        if torch.cuda.is_available():
            input_id = input_id.cuda()
        with torch.no_grad():
            test_question_vector = self.model(input_id) #不输入labels，使用模型当前参数进行预测
            res = torch.mm(test_question_vector.unsqueeze(0), self.knwb_vectors.T)  #计算余弦相似度
...
if __name__ == "__main__":
    ...
    model = TripletLossNetwork(Config)
    model.load_state_dict(torch.load("model_output/epoch_10.pth", weights_only=True))
